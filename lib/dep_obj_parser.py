...
class DepMapUnit:
    ...
    def __init__(self, unit_file: str, parent_unit_path: str, rev_dep: str) -> None:
        ...
        Each DepMapUnit uses the dependency tuple that is passed to it
        to create the reverse dependencies for the units.
        """
        ...
        # Create an attribute for each dep and rev_dep
        for key, value in self.rev_dep_map.items():
            setattr(self, key.lower(), set())
            setattr(self, value, set())
        
        # These are all inferred by other dep/rev_dep types, so they are mapped independently
    ...
    def get_commands(self) -> Set:
        """Return a list of unit file commands.
        
        These commands will later be used to map out executable dependencies.
        """
        return self.commands

    def get_significant_attributes(self, mapping='all') -> List[Union[Tuple[str, str], str]]:
        """Return any attributes that aren't empty.
        
        By default, all attributes containing values will be returned, but users can
        specify only forward or only backward dependencies to be returned as well.
        
        Mapping options:
        'for_deps' - Return a list of tuples containing forward deps and their attributes.
            [ ('key1', 'attr1'), ('key2', 'attr2') ]
        'rev_deps' - Return a list of reverse dependencies/attributes [ 'attr1', 'attr2' ].
            Possible because rev_deps are all lowercase.
        'all' (default) - Return a list of all attributes that contain values.
            [ 'attr1', 'attr2' ]. Discards all method names and empty sets.
        """
        attribute_list = []
        ...
        if mapping == 'for_deps':
            ...
            logging.warning(f'Invalid mapping type for significant attributes.')

        return attribute_list

    def set_rev_dep(self) -> None:
        """Record any reverse dependency being passed when a DepMapUnit object is created."""
    ...
    def load_from_ms(self, master_struct_unit: Dict[str, Union[str, Dict[str, Union[str, List]]]]) -> None:
        """Parse a unit file based on which type of unit it was recorded as."""
    ...
    def update_ms_dep_dir(self, ms_unit_struct: Dict[str, Any]) -> None:
        """Parse dependency directories given by load_from_ms().
        
        '.d' directories don't create dependencies so they are not included in dep objects.
        """
    ...
    def update_ms_sym_link(self, ms_unit_struct: Dict[str, Any]) -> None:
        """Parse symbolic link entries given by load_from_ms().
        
        These are parsed independently to retain the full sym link file path creating the
        dependency, since there can be many sym links pointing to a single file from
        different places within the filesystem.
        """
    ...
    def update_ms_unit_file(self, ms_unit_struct: Dict[str, Any]) -> None:
        """Parse unit file entries given by load_from_ms() for dependencies.
        
        This function is creating most of the dependencies for the entries in the dep map.
        This function is separated from load_dep_map_unit() because unit files actually create
        deps, and loading previous dep map entires only needs to copy the entries that have
        already been recorded. ms_unit_struct can either be full ms_unit_struct or the metadata
        metadata dict within the ms_unit_struct in order to check for implicit dependencies.
        """
        for option in ms_unit_struct:
            try:
                ...
                # Send metadata entries back into the function as a new dictionary
                if option == 'metadata':
    ...
    def load_from_dep_map(self, dep_map_unit: Dict[str, Any]) -> None:
        """Load previously recorded unit file info.
        
        Sister function to load_from_ms(), this function takes an entry already in the dep map and
        records that info to the new dep object. This prevents duplicates in the dep map, and makes
        sure nothing is lost when the current dep object overwrites a previously recorded dep object
        with the same name. This duplication may happen when dep tups have the same dep unit,
    ...
    def create_dep_tups(self, current_item: str) -> List[tuple]:
        """Create a new dependency tuple for any populated dependency attribute.
        
        Check each dependency attribute to see if it is populated. Dependency
        tuples will be created for each populated attribute in order to map
        reverse dependencies to later unit files. This function does not deduplicate
        anything.  Instead, map_dependencies() in systemd_mapping.py keeps track of
        dependency tuple duplicates.
        """
        dep_tup_list = []
    ...
    def record(self) -> Dict[str, Union[str, List[str]]]:
        """Return a dictionary of metadata describing all dependencies for a unit file.
        
        Inspect all of the attributes of the dep object, and if any of them contain info,
        add them to the dict that is returned. The returned dict will be recorded in the
